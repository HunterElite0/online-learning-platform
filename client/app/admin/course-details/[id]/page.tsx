...
async function getCourseReviews(id: number) {
  const URL = "http://localhost:8080/learning/rating/course/" + id;
  const response = await fetch(URL, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
...
async function deleteCourse(id: number) {
  const cookies: any = require("js-cookie");
  const URL = "http://localhost:8080/learning/course/remove/" + id;
  const response = await fetch(URL, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
  ...
  return (
    <main key="1" className="flex flex-col min-h-screen bg-gray-950 text-white">
      <header className="py-4 px-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Link className="flex items-center gap-2" href="/admin/courses">
      ...
      <section className="container mx-auto py-8 px-4 md:px-6 lg:px-8 flex-1">
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input className="hidden disabled:invisible" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <Button className="my-10" onClick={() => deleteCourse(params.id)}>
          Delete Course
        </Button>
      </section>
      <section className="container mx-auto py-8 px-4 md:px-6 lg:px-8 flex-1">
